...
export function validateTimeout(timeout) {
  ...
  if (isNaN(numTimeout) || numTimeout <= 0) return null;
...
/**
 * Normalizes repeated + comma-separated role flags into a clean role list.
 * @param {string[]|undefined} repeatedRoles - Values from repeated --role flags
 * @param {string|undefined} rolesString - Comma-separated roles
 * @returns {string[]} Validated roles
 */
export function normalizeRoles(repeatedRoles = [], rolesString = "") {
  const fromRepeated = Array.isArray(repeatedRoles) ? repeatedRoles : [];
  const combined = [...fromRepeated];
  if (rolesString) combined.push(...rolesString.split(","));
  return validateRoles(combined.join(","));
}

/**
 * Normalizes repeated + comma-separated skill flags into a clean skill list.
 * @param {string[]|undefined} repeatedSkills - Values from repeated --skill flags
 * @param {string|undefined} skillsString - Comma-separated skills
 * @returns {string[]} Normalized skills
 */
export function normalizeSkills(repeatedSkills = [], skillsString = "") {
  const fromRepeated = Array.isArray(repeatedSkills) ? repeatedSkills : [];
  const combined = [...fromRepeated];
  if (skillsString) combined.push(...skillsString.split(","));
  return [...new Set(
    combined
      .map((skill) => (typeof skill === "string" ? skill.trim().toLowerCase() : ""))
      .filter((skill) => skill.length > 0 && skill.length <= 64),
  )].slice(0, 10);
}
...
/**
 * Parses human-readable durations such as 30m, 2h, or 1d into seconds.
 * @param {string|number|undefined} value - Duration value
 * @returns {number|null} Parsed duration in seconds or null
 */
