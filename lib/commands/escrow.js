...
export async function createEscrow(toAgentId, amount, opts) {
  ...
  const timeout = opts.timeout ? validateTimeout(opts.timeout) : undefined;
  if (opts.timeout && timeout === null) {
    console.error("Timeout must be a whole number from 1 to 10080 minutes.");
    process.exit(1);
  }
...
export async function releaseEscrow(escrowId, opts) {
  ...
  if (!isValidEscrowId(escrowId)) {
    console.error(validationError("escrow ID", "8-64 alphanumeric characters, dashes and underscores"));
    process.exit(1);
  }

  if (opts.recipient && !isValidAgentId(opts.recipient)) {
    console.error(validationError("recipient agent ID", "3-64 alphanumeric characters, dashes and underscores"));
    process.exit(1);
  }
