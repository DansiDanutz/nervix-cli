...
import { trpcMutation, refreshAccessToken } from "../api.js";
...
export async function start(opts) {
  ...
  async function beat() {
    try {
      ...
      if (e.message && (e.message.includes("expired") || e.message.includes("Invalid agent token"))) {
        console.log(`\n[REFRESH] Token expired, refreshing...`);
        const ok = await refreshAccessToken();
        if (ok) {
          console.log(`[REFRESH] New token obtained, retrying heartbeat...`);
          try {
            const result = await trpcMutation("agents.heartbeat", {
              ipAddress: opts.ip || undefined,
              region: opts.region || undefined,
            ...
            total++;
            const ts = result.timestamp || new Date().toISOString();
            process.stdout.write(`\r[${ts}] heartbeat #${total} ok (refreshed)`);
            return;
          } catch (retryErr) {
            console.error(`\n[ERROR] Heartbeat failed after refresh: ${retryErr.message}`);
          }
        } else {
          console.error(`[REFRESH] Failed — re-enroll may be needed`);
        }
      } else {
